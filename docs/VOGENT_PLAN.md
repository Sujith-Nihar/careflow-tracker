...
| A function input `value` is where the model reasons | **it is a template, not a prompt.** `{{node.x.status}}` is substituted; a conditional written there is never evaluated and arrives as literal text. | V2 filed the wrong disposition on 3 of 4 scenarios (`INVESTIGATIONS.md` INV-5) |
