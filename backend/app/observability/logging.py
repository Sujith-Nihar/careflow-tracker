...
ALLOWED_FIELDS = frozenset(
    {
        ...
        # correlation
        "request_id", "organization_id", "call_id", "dial_id", "action_execution_id",
        "scenario_id", "evaluation_run_id", "job_id", "versioned_prompt_id", "vogent_agent_id",
        # classification and outcome, all enum-like
        "function", "kind", "outcome", "status", "lifecycle", "severity", "mode", "strategy",
        "duplicate", "attempt_no", "latency_ms", "http_status", "reason_code", "event_type",
        "requires_staff_action", "promise_mismatch", "connected_seconds", "system_result_type",
        "passed", "receive_count", "count", "rules_version", "error_type",
    }
)
...
DENIED_FIELDS = frozenset(
    {
        "params", "payload", "request_payload", "response_payload", "transcript",
        "evidence_text", "callback_phone", "patient_ref", "concern_summary", "reason",
        "summary", "token", "api_key", "authorization", "dial", "note", "actor",
    }
)
...
def _merge_context(_logger, _name, event_dict: dict) -> dict:
    return {**_context.get(), **event_dict}
